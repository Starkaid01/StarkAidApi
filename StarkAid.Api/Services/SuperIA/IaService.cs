...
namespace StarkAid.Api.Services.SuperIA
{
    public class IaService
    {
        ...
        public async Task<(bool Sucesso, string Texto)> ChamarStarkNlp(string fraseOriginal)
        {
            try
            {
                ...
                if (config == null || string.IsNullOrWhiteSpace(config.StarkNlpUrl))
                    return (false, "URL do StarkNLP não configurada no sistema.");

                // Encode da frase
                var encoded = HttpUtility.UrlEncode(fraseOriginal);

                // Monta a URL completa usando apenas a base + endpoint fixo
                var endpoint = "/random-answers";
                var url = $"{config.StarkNlpUrl.TrimEnd('/')}{endpoint}?resposta={encoded}";

                var response = await _httpClient.GetAsync(url);
                if (!response.IsSuccessStatusCode)
                {
                    var err = await response.Content.ReadAsStringAsync();
                    _logger.LogError("Erro StarkNLP: {0}", err);
                    return (false, $"Erro do servidor NLP: {err}");
                }

                var json = await response.Content.ReadAsStringAsync();
                using var doc = JsonDocument.Parse(json);
                var alternativas = doc.RootElement.EnumerateArray().Select(e => e.GetString()).ToArray();
                ...
                var jsonFormatado = JsonSerializer.Serialize(new { alternativas }, options);
                return (true, jsonFormatado);
            }
            catch (Exception ex)
            {
        ...
        // dentro de public class IaService
        public async Task<IaResultado?> ProcessarMensagemJson(object[] mensagens)
        {
            // Tenta OpenRouter primeiro (já lida com falhas retornando null)
