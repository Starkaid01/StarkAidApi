            ...
            string? existingCustomerId = null)
        {
            try
            {
                ...
                try
                {
                    ...
                    _logger.LogInformation("✅ Price ID {PriceId} encontrado. Produto: {ProductId}, Valor: {Amount} {Currency}", 
                        priceId, price.ProductId, price.UnitAmount, price.Currency);
                    
                    // Verificar se o produto está ativo
                    if (price.Active == false)
                    {
                        _logger.LogWarning("⚠️ Price ID {PriceId} existe mas está INATIVO!", priceId);
                        throw new InvalidOperationException($"O Price ID '{priceId}' existe mas está inativo no Stripe. Por favor, ative-o no Stripe Dashboard.");
                    }
                }
                catch (StripeException ex) when (ex.Message.Contains("No such price"))
                {
                    _logger.LogError("❌ Price ID {PriceId} NÃO EXISTE no Stripe. Verifique se está usando o ambiente correto (Test/Live) e se o Price ID está correto.", priceId);
                    throw new InvalidOperationException(
                        $"O Price ID '{priceId}' não existe no Stripe.\n" +
                        $"Verifique:\n" +
                        ...
                        $"Chave atual: {_stripeSettings.SecretKey?.Substring(0, Math.Min(20, _stripeSettings.SecretKey?.Length ?? 0))}...", ex);
                }
