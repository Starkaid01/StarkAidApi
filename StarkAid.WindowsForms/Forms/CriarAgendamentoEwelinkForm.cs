...
public partial class CriarAgendamentoEwelinkForm : Form
{
    ...
    private List<EwelinkDevice> _devices = new();

    public CriarAgendamentoEwelinkForm(ApiService apiService)
    {
    ...
    {
        this.Text = "Criar Agendamento Ewelink";
    ...
    {
        _devices = await _apiService.GetEwelinkDevicesAsync();
        _cmbDevice!.Items.Clear();
        foreach (var device in _devices)
        {
            _cmbDevice.Items.Add($"{device.Name} ({device.DeviceId})");
        }
    ...
    private async void BtnSalvar_Click(object? sender, EventArgs e)
    {
        ...
        {
            MessageBox.Show("Selecione um dispositivo Ewelink!", "Aviso", MessageBoxButtons.OK, MessageBoxIcon.Warning);
            return;
        }
        ...
        try
        {
            var agendamento = await _apiService.CreateAgendamentoEwelinkAsync(
                device.DeviceId,
                acao,
                data,
                hora,
            ...
            if (agendamento != null)
            {
                ...
                MessageBox.Show("Agendamento Ewelink criado com sucesso!", "Sucesso", MessageBoxButtons.OK, MessageBoxIcon.Information);
            ...
            else
            {
                ...
                MessageBox.Show("Erro ao criar agendamento Ewelink!", "Erro", MessageBoxButtons.OK, MessageBoxIcon.Error);
            }
        }
        catch (Exception ex)
        {
